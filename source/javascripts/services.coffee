...
app.factory "placesService", ($http) ->
  getPath = (path, params) ->
    $http(
      url: apiHost + path
      method: 'GET'
...
PlacesCtrl = ($scope, placesService, $timeout) ->
...
w.PlacesCtrl = PlacesCtrl

PostalCodesCtrl = ($scope, placesService) ->
  postalCodesLimit = 80
  $scope.loading = true
...
w.PostalCodesCtrl = PostalCodesCtrl

CitiesCtrl = ($scope, placesService) ->
  $scope.loading = true
